...
feature "broadcasting message" do
  ...
  let!(:posting_chatroom_user) { create(:chatroom_user, chatroom: chatroom, user: posting_user) }

  scenario "successfully", :js => true do
    sign_in(user)
    visit chatroom_path(chatroom)
    expect(page).not_to have_content("this is a message")
    message = Message.create(body: "this is a message", user: posting_user, chatroom: chatroom)
